...
class TrainingDataSet(object):
    def __init__(self, sliding_dir, it_path, batch_size,pth_clip_sentence_pairs_iou):

        self.counter = 0
        self.batch_size = batch_size
        self.context_num = 1
        self.context_size = 128
        self.pth_clip_sentence_pairs_iou = pth_clip_sentence_pairs_iou
        print ("Reading training data list from "+it_path)
        cs = load_obj(it_path)#pickle.load(open(it_path.encode()))
        movie_length_info = load_obj("./video_allframes_info.pkl")#pickle.load(open("./video_allframes_info.pkl"))
        ...
        try:
            self.clip_sentence_pairs_iou = load_obj(self.pth_clip_sentence_pairs_iou)
        except:
        # read sliding windows, and match them with the groundtruths to make training samples
            ...
            save_obj(self.pth_clip_sentence_pairs_iou,self.clip_sentence_pairs_iou)
...
class TestingDataSet(object):
    def __init__(self, img_dir, csv_path, batch_size):
        ...
        csv = load_obj(csv_path)#pickle.load(open(,'wb'))
        self.clip_sentence_pairs = []
        for l in csv:
            clip_name = l[0]
            sent_vecs = l[1]
        ...
        with open("test_clip_names.pkl",'wb')as f:
            pickle.dump(self.sliding_clip_names,f)
